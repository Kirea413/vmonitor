...
- **複数台を同時接続** — Wi-Fi/USBを合わせて最大8セッションを独立管理します。
  先着1台は仮想拡張画面、追加端末にはメイン画面のミラーを配信します
...
| 接続 | USB ケーブル、または PC と同じ Wi-Fi（iOS USB は `iproxy` が必要） |
...
2. Wi-Fiならスマホ側の一覧からPCを選んで「接続」を押す
3. PCに出る確認を許可する（次回から同じ端末として認識されます）
4. 複数台使う場合は、別の端末でも2〜3を繰り返す

AndroidのUSB直結と、PC側からスマホへ接続する従来の向きも利用できます。
複数台接続では、端末側からPCへ接続する向きを推奨します。
...
ワークフローは解析とテストを通した後、IPAとSHA-256ファイルを生成します。

## 分かっている制限

- 検証した環境が上の表のとおり限られています
- iOS で確認できているのは iPhone 17 の 1 台だけです。iOS の USB 接続は
  `usbmuxd` / `iproxy` を使う試験対応です。PCがUSB制御路を先に待機し、
  iPhone側の「USBで接続」から開始できますが、実機検証はまだです
- 現行の仮想ディスプレイドライバが作れる拡張画面は1枚です。複数台を同時に
  接続した場合、2台目以降はPCのメイン画面をミラーします
- 通常のスマホ→PC Wi-Fi接続はTLSで暗号化し、初回に見たPC証明書を端末へ
  保存して再接続時に照合します。PC→スマホの手動接続は互換用の平文TCPなので、
  信頼できるローカルネットワーク内だけで使用してください
- 拡大率は端末によっては指定どおりにならず、Windows が近い段階へ丸めます

## オープンソース ライセンス

vmonitorは、Flutter、.NET、LibUsbDotNet、Vortice.Windows、Makaretu.Dns、
UsbDk、libusbmuxdなどのオープンソースソフトウェアを利用しています。
各プロジェクトの開発者・貢献者の皆様に感謝します。

- PCアプリ: 「ライセンス」タブで、利用ライブラリ一覧と配布条件を確認できます
- Android / iOSアプリ: ホーム画面右上の情報ボタン、または設定画面の
  「オープンソース ライセンス」から確認できます
- 配布物の著作権表示と取得元: [THIRD-PARTY-NOTICES.md](THIRD-PARTY-NOTICES.md)
