...
| スマホ | Android 7.0 (API 24) 以上 / iOS 13.0 以上（**未検証**） |
...
| iOS 実機 | ❌ **完全に未検証**。ビルドが通っただけ |
| 自動テスト | .NET 170 件、Flutter 72 件 |
...
- iOS 版は動作確認がありません。USB 直結は iOS では原理的にできません
- 拡大率は端末によっては指定どおりにならず、Windows が近い段階へ丸めます
- インストーラーに含まれる `VMonitorSetup.exe` が .NET ランタイムを二重に
  抱えており、約 64 MB の無駄があります（動作には影響しません）
